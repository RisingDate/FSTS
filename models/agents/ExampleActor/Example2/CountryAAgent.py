...
"""
国家AAgent：代表国家A国家行为体，基于新的属性参数设计
...
class CountryAAgent(LLMAgent):
    def __init__(self, agent_name='CountryAAgent', llm_model='deepseek-r1:32b-qwen-distill-q8_0'):
        ...
        self.identify = "国家（大国）"
        self.age = 249
        self.military_strength = 0.82
        self.economic_resilience = 0.8
        self.tech_level = 0.9
        self.leadership_risk_aversion = 0.2
        self.leadership_communication_willingness = 0.05
        self.leadership_decision_speed = 0.8
        self.domestic_political_pressure = 0.7
        self.hardline_tendency = 0.95
        self.nuclear_deterrence = 0.95
        self.backchannel_quality = 0.2
        self.third_party_mediation = 0.05
        self.intelligence_reliability = 0.6
        self.proximity_factor = 0.9
        self.alliance_influence = 0.85
        self.signal_noise_level = 0.5
        self.crisis_duration = 0.1
        self.ally_intervention_likelihood = 0.3
        ...
        self.system_prompt = '''
            你是国家A国家行为体，代表国家A在国际危机中的立场和决策。
            
            你的核心特征：
            - 强大的军事力量（0.82）和经济韧性（0.8）
            - 先进的技术水平（0.9）和联盟影响力（0.85）
            - 强大的核威慑能力（0.95）
            - 较低的风险规避（0.2）但决策速度快（0.8）
            - 强硬倾向很高（0.95）
            - 较低的沟通意愿（0.05）和后渠道质量（0.2）
            - 中等的情报可靠性（0.6）
    ...
    def update_attributes(self, external_influence=None, autonomous_change=False):
        ...
        if autonomous_change:
            ...
            if self.hardline_tendency > 0.8:
                tension_change += random.uniform(0, 0.03)
            ...
            if self.current_international_tension > 0.7:
                risk_change = random.uniform(-0.02, 0.05)  # 高紧张时可能更谨慎
            else:
                risk_change = random.uniform(-0.03, 0.02)
            ...
            # 3. 沟通意愿根据危机持续时间调整
            if self.crisis_duration > 0.5:
                comm_change = random.uniform(0, 0.03)  # 长期危机可能增加沟通意愿
            else:
                comm_change = random.uniform(-0.02, 0.02)
